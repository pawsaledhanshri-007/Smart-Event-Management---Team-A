...
def test_event_creation(client, admin_token, db):
    ...
    venue_response = client.post(f"{settings.API_V1_STR}/venues/", json={"name": "Event Hall", "capacity": 10, "location": "LA"}, headers=headers)
...
def test_registration(client, user_token, admin_token):
    ...
    # Create venue
    venue = client.post(f"{settings.API_V1_STR}/venues/", json={"name": "Reg Hall", "capacity": 5, "location": "SF"}, headers=admin_headers).json()
    ...
    event = client.post(f"{settings.API_V1_STR}/events/", json=event_data, headers=admin_headers).json()
    
    # Register user
    response = client.post(f"{settings.API_V1_STR}/events/{event['id']}/register", headers=user_headers)
    assert response.status_code == 201
    assert response.json()["status"] == "REGISTERED"

    # Duplicate registration prevention
    response_duplicate = client.post(f"{settings.API_V1_STR}/events/{event['id']}/register", headers=user_headers)
    assert response_duplicate.status_code == 409

    # Capacity validation
    # Admin tries to register (as a different user) to hit capacity
    response_capacity = client.post(f"{settings.API_V1_STR}/events/{event['id']}/register", headers=admin_headers)
    assert response_capacity.status_code == 422
    
    # Registration cancellation
    response_cancel = client.delete(f"{settings.API_V1_STR}/events/{event['id']}/register", headers=user_headers)
